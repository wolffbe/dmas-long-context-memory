...
app = FastAPI(title="LOCOMO Chat Container", version="1.0")

class DialogTurn(BaseModel):
    speaker: str
    dia_id: str
    text: str
    img_url: Optional[Any] = None  # Can be string or list
    blip_caption: Optional[Any] = None  # Can be string or list

class Question(BaseModel):
    question: str
    answer: Optional[Any] = None  # Can be int, str, or missing
    adversarial_answer: Optional[Any] = None  # Alternative answer field
    category: Optional[Any] = None  # Can be int or str in LOCOMO dataset
    evidence: Optional[List[str]] = None
    
    def get_answer(self) -> str:
        """Get the answer, checking both possible fields"""
...
class EventSummary(BaseModel):
    """Event summary can have various structures in LOCOMO"""
    pass  # Flexible structure, will store as dict

class Conversation(BaseModel):
    sample_id: str
    speaker_a: str
    speaker_b: str
    sessions: Dict[str, List[DialogTurn]]  # session_1, session_2, etc.
    session_datetimes: Dict[str, str]  # session_1_date_time, etc.
    observations: Optional[Dict[str, Any]] = None  # session_1_observation, etc. - flexible
    session_summaries: Optional[Dict[str, Any]] = None  # session_1_summary, etc. - flexible
    event_summary: Optional[Dict[str, Any]] = None  # events_session_1, etc. - flexible structure
...
class ChatStorage:
    """Loads and stores LOCOMO conversations"""
    
    def __init__(self, data_url: str):
        self.data_url = data_url
        self.data_path = Path("/data/locomo10.json")  # Fixed path
        self.conversations = []  # List of Conversation objects
        self.conversations_by_id = {}  # Dict[sample_id, Conversation]
        self.all_sessions = []  # Flattened list of all sessions with metadata
        self.all_questions = []  # Flattened list of all questions
    ...
    def load_from_json(self):
        """
        Load conversations from LOCOMO JSON file
        
        Expected structure matches locomo10.json format with:
        - sample_id
        - speaker_a, speaker_b
        - session_1, session_2, ..., session_N (each is list of dialog turns)
        - session_1_date_time, session_2_date_time, ...
        - session_1_observation, session_2_observation, ... (optional)
        - session_1_summary, session_2_summary, ... (optional)
        - event_summary with events_session_1, events_session_2, ... (optional)
        - qa (optional list of questions)
        """
        ...
        try:
            ...
            for conv_idx, conv_data in enumerate(data):
                try:                    
                    # Sessions are nested inside 'conversation' key
                    conversation_data = conv_data.get("conversation", {})
                    
                    # Get speaker names - check both top level and nested
                    ...
                    # Extract sessions from the nested conversation object
                    sessions = {}
                    session_datetimes = {}
                    observations = {}
                    session_summaries = {}
                    ...
                    for key, value in conversation_data.items():
                        # Check for session data (session_1, session_2, etc.)
                        # Must start with "session_" but NOT end with special suffixes
                        if key.startswith("session_"):
                            ...
                            # This is actual session dialog data
                            elif isinstance(value, list):
                                sessions[key] = [DialogTurn(**turn) for turn in value]
                        
                        elif key.startswith("events_session_"):
                            # Store event summaries as-is (they have speaker names as keys)
                            event_summary[key] = value
                        
                    # Handle top-level fields (observation, session_summary, event_summary)
                    ...
                    # Flatten sessions for easy access
                    ...
                    if qa:
                        for idx, question in enumerate(qa):
                            q_dict = {
                                ...
                                "answer": question.get_answer(),  # Use helper method
    ...
    def get_conversation(self, sample_id: str) -> Optional[Conversation]:
        """Get a conversation by sample_id"""
    ...
    def get_conversation_session_by_id(self, sample_id: str, session_id: str) -> Optional[Dict]:
        """Get a specific session by session_id from a conversation"""
    ...
    def get_conversation_session(self, sample_id: str, session_index: int) -> Optional[Dict]:
        """Get a specific session by index from a conversation"""
        conversation = self.get_conversation(sample_id)
        if not conversation:
            return None
        
        # Get session keys sorted (session_1, session_2, etc.)
    ...
    def get_conversation_session_by_conv_index(self, conv_index: int, session_index: int) -> Optional[Dict]:
        """Get a specific session by conversation index and session index"""
    ...
    def get_sessions(self, sample_id: Optional[str] = None) -> List[Dict]:
        """Get sessions, optionally filtered by sample_id"""
        if sample_id:
...
async def startup_event():
    ...
    print("LOCOMO Chat Container starting...")
...
async def get_conversations(skip: int = 0, limit: int = 10):
    ...
    - **skip**: Number to skip
    - **limit**: Maximum to return (max 10)
    """
...
async def get_conversation(sample_id: str):
    """
    Get a specific conversation by sample_id - conversation sessions only
    ...
    # Return only core conversation data
    return {
        "sample_id": conversation.sample_id,
...
async def get_conversation_by_index(index: int):
    """
    Get a conversation by index (0-based) - conversation sessions only
...
async def get_conversation_session_by_id(sample_id: str, session_id: str):
    ...
    - **sample_id**: Conversation identifier (e.g., "conv-26")
    - **session_id**: Session identifier (e.g., "session_1", "session_2")
    """
    if not storage.is_loaded:
        raise HTTPException(status_code=503, detail="Data not loaded")
...
async def get_conversation_session_by_index(conv_index: int, session_index: int):
    ...
    - **conv_index**: Conversation index (0-based)
    - **session_index**: Session index (0-based, e.g., 0 = session_1, 1 = session_2)
    """
    if not storage.is_loaded:
        raise HTTPException(status_code=503, detail="Data not loaded")
...
async def get_questions_by_conversation_index(index: int):
    ...
    - **index**: Conversation index (0-based)
    """
    if not storage.is_loaded:
        raise HTTPException(status_code=503, detail="Data not loaded")
